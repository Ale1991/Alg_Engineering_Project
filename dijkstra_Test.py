...
def dynDijkstraTest(g, experiment_number, missing_edge_to_add):
    ...
    for i in range(experiment_number):
        ...
        # first dijkstra computation without adding missing edge
        sssp = networkit.distance.DynDijkstra(g, 0)
        sssp.run()
...
def getMissingEdgeRandomlyFromGraph(graph, edge_number):
    ...
    print(
        f"archi da trovare: {edge_number}, archi trovati: {len(missingEdges)}")
    n = graph.numberOfNodes()
    e = graph.numberOfEdges()
    print(
        f'#node:{n}, #edge:{e}, #theoreticallyMissingEdges:{(n * (n - 1) / 2) - e}, #missingEdges added:{len(missingEdges)}')
    return missingEdges

def getRandomGraphEdgeEvent():
    graph_event_list = [networkit.dynamic.GraphEvent.EDGE_ADDITION,
                        networkit.dynamic.GraphEvent.EDGE_REMOVAL,
                        networkit.dynamic.GraphEvent.EDGE_WEIGHT_INCREMENT,
                        networkit.dynamic.GraphEvent.EDGE_WEIGHT_UPDATE]
...
if __name__ == "__main__":
    ...
    n0 = 0
    batagelj = True
    graph = networkit.generators.BarabasiAlbertGenerator(
        k, nMax, n0, batagelj).generate()

    # 10% del numero totale di archi
    edgeToAddDynamically = int(0.8 * nMax * k)
...
# grafici con running time (ordinate) ascisse ( vertici,archi, densita)
    ...
    print(f"Static Dijkstra with {experiment_number} experiments ended in {static_end_time - static_start_time} seconds")
    ...
    print(
        f"Dynamic Dijkstra with {experiment_number} experiments ended in {dyn_end_time - dyn_start_time} seconds")
    print(
        f"Dijkstra computed foreach dinamically-added edge, total: {len(randomMissingEdgeList)}")

    # SPEEDUP: rapporto tra tempo di esecuzione dell' algo piu' lento e quello piu' veloce
    speed_up = int((static_end_time - static_start_time) /
                   (dyn_end_time - dyn_start_time))
    print(
        f"DynDijkstra e' {speed_up} volte piu' veloce rispetto a StaticDijkstra")
